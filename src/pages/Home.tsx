...
export default function Home() {
  ...
  const contests = [
    { title: "Fly.. Fly! Arena", author: "Tonny Birs", reward: "$5.00" },
    { title: "Singing Arena", author: "Cecilia Wong", reward: "$5.00" },
    { title: "Dance Arena", author: "Emma Johnson", reward: "$5.00" }
  ];
  ...
  return (
    <div className="flex flex-col min-h-screen w-full bg-gray-50">
      {/* Header */}
      <header className="sticky top-0 z-50 w-full bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between max-w-[93rem] mx-auto">
          <div>
            <p className="text-sm text-gray-600">{getTimeOfDay()}</p>
            <h1 className="text-lg font-semibold text-purple-500">Nate</h1>
          </div>
      ...
      <main className="flex-1 w-full container mx-auto px-4 py-8 overflow-x-hidden lg:w-full">
          ...
          <Card className="bg-blue-500 text-white overflow-hidden">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                ...
                <img
                  src="/public/placeholder.svg?height=100&width=100"
                  alt=""
                  className="h-24 w-24"
                />
              </div>
        ...
        <section className="mb-8">
          <h2 className="text-xl font-bold mb-4 text-center">New Contests</h2>
                ...
                <CarouselItem key={index} className="pl-2 md:pl-4 md:basis-1/2 lg:basis-1/3">
                  <Card className="overflow-hidden">
                    <CardContent className="p-0">
                      <img
                        src={`/public/placeholder.svg?height=200&width=400&text=${contest.title}`}
                        alt={contest.title}
                      ...
                      <div className="p-4">
                        ...
                        <p className="text-sm text-gray-500">By {contest.author}</p>
                        <p className="text-orange-500 font-semibold mt-2">{contest.reward}</p>
                      </div>
                    </CardContent>
                  </Card>
                </CarouselItem>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex gap-4">
                <img
                  src="/public/placeholder.svg?height=100&width=100&text=Super+Redly+Gear"
      ...
      {/* Bottom Navigation */}
      <nav className="sticky bottom-0 w-full bg-white border-t p-4">
        <div className="flex justify-around items-center max-w-md mx-auto">
          {["Home", "Contest", "Ranking", "Rewards"].map((item, index) => (
            <Button key={item} variant="ghost" size="icon" className="flex flex-col items-center gap-1">
              <div className={`h-6 w-6 ${index === 0 ? 'bg-blue-500' : 'bg-gray-300'} rounded-lg`} />
